...
| **2026-02-27** | **SQLite-backed LLM response cache — 352 tests** | `database.py` adds `llm_response_cache` table; `b0_intake_agent._call_llm()` wraps every Foundry/OpenAI call with SHA-256 cache key (`tier :: model :: prompt`); cache hit skips API call entirely; eliminates repeat latency (p50 3–5 s → ~0 ms on cache hit); suite grown 342 → **352 passing** |
