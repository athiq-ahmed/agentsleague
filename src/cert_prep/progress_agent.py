...
_DOMAIN_WEIGHT: dict[str, float] = {d["id"]: d["weight"] for d in AI102_DOMAINS}
_DOMAIN_NAME:   dict[str, str]   = {d["id"]: d["name"]   for d in AI102_DOMAINS}
...
class DomainProgress:
    """Student's self-assessed progress on one AI-102 domain (1–5 scale)."""
    domain_id:    str
    domain_name:  str
...
class ProgressAgent:
    ...
    def assess(
        ...
        for dp in snap.domain_progress:
            w = _DOMAIN_WEIGHT.get(dp.domain_id, 1 / len(AI102_DOMAINS))
    ...
    def _build_nudges(
        ...
        if (snap.done_practice_exam == "yes"
            ...
            nudges.append(Nudge(
                ...
                message=(
                    "Microsoft AI-102 requires ~70% to pass. Your practice score suggests "
...
def generate_weekly_summary(
    ...
    <body style="font-family:Segoe UI,Arial,sans-serif;max-width:640px;margin:auto;
      ...
      <p style="margin-top:24px;font-size:0.8rem;color:#888;text-align:center;">
        Generated by <b>AI-102 Cert Prep Agent</b> · Microsoft Agents League ·
